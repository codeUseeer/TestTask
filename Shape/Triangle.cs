...
namespace Geometry
{
	public class Triangle : ITriangle
	{
		...
		{
			if (edgeA < eps)
				throw new ArgumentException("Incorrect triangle side", nameof(edgeA));

			if (edgeB < eps)
				throw new ArgumentException("Incorrect triangle side", nameof(edgeB));

			if (edgeC < eps)
				throw new ArgumentException("Incorrect triangle side", nameof(edgeC));
			...
			if ((perimeter - maxEdge) - maxEdge < Constants.CalculationAccuracy)
				throw new ArgumentException("Incorrect triangle sum");
